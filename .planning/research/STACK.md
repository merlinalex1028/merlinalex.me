...
**Domain:** Personal blog + portfolio with 沉浸二次元 (immersive anime/kawaii) atmosphere
**Researched:** 2026-06-02
**Confidence:** HIGH (versions verified via Context7 + official GitHub releases on 2026-05-28; ecosystem mapping cross-referenced across Hexo themes directory + project repos)

---

## TL;DR Recommendation

**Use Astro v6 + Tailwind v4 + l2d-widget + APlayer/MetingJS + tsParticles + Twikoo, deployed to Cloudflare Pages.**

The PROJECT.md atmosphere requirements (ATM-01..06) are all *custom interactive features* — a Live2D mascot with dress-up, theme-linked falling petals, custom right-click menu, BGM with resume, easter eggs. Even the closest pre-built Hexo themes (Butterfly/Anzhiyu) only ship a subset; you will be writing custom JS regardless. Astro gives you a modern islands runtime to host that custom JS cleanly, plus content collections perfectly map to the multi-module content (articles / projects / creations / microblog / anime list / timeline). Hexo's theme advantage shrinks when the atmosphere is the differentiator and must be hand-built anyway.

Comparison decision: **Astro WINS over Hexo on DX/flexibility/integration story.** Hexo wins only on "one-command get an existing 二次元 look" — which conflicts with the project's "uniquely mine" core value.

Hugo is **ruled out** — Go templating is awkward for interactive islands, and its theme ecosystem has essentially zero 二次元 entries.
...
**Verdict for THIS project:** Astro. The "沉浸二次元 atmosphere" is custom work either way (since the project wants its *own* identity, not Butterfly-clone-#12,847), so Astro's superior DX, content modeling, and zero theme lock-in dominate.
...
**Confidence: HIGH** (verified Cloudflare Pages limits doc, accessed 2026-06-02).
...
pnpm create astro@latest merlinalex-site -- --template basics --typescript strict --no-git
cd merlinalex-site
pnpm astro add tailwind            # Tailwind 4 via Vite plugin
pnpm astro add mdx
...
# Atmosphere libs
pnpm add l2d-widget
pnpm add @tsparticles/engine @tsparticles/slim @tsparticles/preset-snow
pnpm add aplayer
...
# Deployment
pnpm add -D wrangler
pnpm add @astrojs/cloudflare   # if any pages need SSR; otherwise pure static
```
...
**If owner decides ATM-01 (Live2D) is too heavy or hurts mobile:**
...
**If owner wants to ship MVP in <1 weekend and skip atmosphere customization:**
...
**If site grows past ~500 articles or owner adds collaborators:**
- Same Astro stack
...
**If owner adds e-commerce or accounts (currently Out of Scope per PROJECT.md):**
...
*Stack research for: 二次元 personal blog + portfolio*
*Researched: 2026-06-02*
